...
4. **第三方實測（陳仁暉教授建議）**：目前所有檢驗都是團隊自己設計的。
   ...
   作為第三方的驗證證據。與待辦第 13 項互補：第 13 項量的是判定準不準，本項量的是使用者用不用得起來。設計時應先訂出「達成」的定義（例如能否在 3 分鐘內完成一次查證
...
6. **後端防休眠**：GitHub Actions 的 keepalive 實際每 2–5 小時才執行，Render 仍會休眠
   （前端已能在喚醒期間自動重試；2026-09-23 起熱門頁與知識庫頁的列表也會在後端醒來後自動重抓，
   先前只有橫幅會消失、列表停在「暫時連不上伺服器」，要按「重試」才恢復）。可行方案是在 Supabase 以 `pg_cron` 搭配 `pg_net` 每 10 分鐘
