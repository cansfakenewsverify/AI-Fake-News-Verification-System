...
2. **自評表與評分表**：已照指導教授發的空白表原格式做成可直接列印的 `09_學生自評表_已填.pdf`（3 頁，九項等級與說明都填好）與 `11_評分表_已填表頭.pdf`（4 頁，分數留給老師）。產生器 `presentations/tools/build_course_forms.mjs`，要改等級或說明改那裡重跑。
