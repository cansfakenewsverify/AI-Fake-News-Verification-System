...
4. **雲端知識庫的來源標記**：2026-09-20 檢查 `/api/knowledge` 發現 128 筆中有 24 筆的查核來源
   實為求證平台上尚未有判定的回報貼文，卻標記為 Tier 1 查核機構，與「只有已做出判定的來源才算來源」
   的原則不符，需清除或重新標記。簡報影片已避開這些列。
5. **後端防休眠**：GitHub Actions 的 keepalive 實際每 2–5 小時才執行，Render 仍會休眠
   （前端已能在喚醒期間自動重試）。可行方案是在 Supabase 以 `pg_cron` 搭配 `pg_net` 每 10 分鐘
   呼叫 `/health`；此為在正式資料庫新增常駐設定，須先取得測試負責人同意。
6. **雲端知識庫補入示範用的 gold 列**：細節見 `CLAUDE.md` 第 8 節。
7. **UI-6**：OpenCC 簡繁比對，其中英文字允許清單須經指導教授裁定。
8. **熱門牆資料更新**：雲端排程關閉，目前為手動更新，資料更新時間停在 2026-07-12。
9. **Threads 實機串接**：排在報告後，需 Meta App Review 與企業驗證，詳見 `CLAUDE.md` 第 11 節。
