...
## This project: demo_v0.4.0 (全民查證公社 progress-report demo film)

- **Local only.** Render on this machine. Do not run `hyperframes publish`, `cloud`, `lambda`, `cloudrun`, `auth` or
  `feedback`; do not sign in anywhere. Run the CLI with `HYPERFRAMES_NO_TELEMETRY=1` (or `DO_NOT_TRACK=1`).
- Skills are installed **project-level** (`.claude/skills`, `.agents/skills`, both git-ignored). `npx hyperframes skills
  update` installs into the user's home directory instead; to refresh them here use
  `npx skills add https://github.com/heygen-com/hyperframes --skill <name> --agent claude-code universal --copy --full-depth --yes`.
- Source of truth: `BRIEF.md` → `STORYBOARD.md` → `frame.md` → `compositions/`. `data/plan.json` is the edit decision
  list; `index.html`, `compositions/captions.html`, `compositions/chrome.html` and `docs/demo/demo_v0.4.0.srt` are
  generated by `node tools/build_timeline.mjs`.
- Honesty rules (BRIEF.md → Notes) are part of the design: every UI frame is a genuine capture, the Threads part is
  labelled 模擬模式, accelerated footage is labelled 畫面加速, the suspicious URL is a mask block with no text under it.
- Media is git-ignored. Rebuild it with `node tools/build_footage.mjs <rawDir>` (v0.3.0 captures) and `npm run tts`.
...
npm run render       # render to MP4 (local)
