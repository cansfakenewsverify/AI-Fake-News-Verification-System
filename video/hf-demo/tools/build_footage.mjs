// Rebuilds the footage clips used by the compositions from the genuine v0.3.0 screen captures.
//
// Usage (from video/hf-demo):  node tools/build_footage.mjs <rawDir> [name ...]
//   rawDir = folder with the v0.3.0 takes (S2_t1/, S56_t1/, S7_t3/, S8_t1/, S9_t2/, each with frames/meta.json).
//            For the 2026-09-16 recordings this is the session scratchpad: …/scratchpad/video/raw
//
...
if (!rawDir || !fs.existsSync(rawDir)) {
  console.error("usage: node tools/build_footage.mjs <rawDir> [name ...]");
  process.exit(1);
}
