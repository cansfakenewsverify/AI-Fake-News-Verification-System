// Assembles the film from data/plan.json (edit decision list) + data/timing.json (real narration timings).
//
...
//   ../../docs/demo/demo_v0.4.0.srt the same cues as a sidecar subtitle file
...
<html lang="zh-Hant" data-resolution="landscape">
  <head>
    ...
    <title>全民查證公社 demo v0.4.0</title>
    ...
    <template>
      ...
      <div id="root" class="fnv-root" data-composition-id="chrome" data-width="1920" data-height="1080" data-duration="${TOTAL}">
        <div class="chr-brand" id="chr-brand">
          ...
          <span class="chr-sub fnv-mono">進度報告 DEMO v0.4.0</span>
        </div>
...
const srtPath = path.resolve(ROOT, "..", "..", "docs", "demo", "demo_v0.4.0.srt");
fs.writeFileSync(srtPath, srt, "utf8");

fs.writeFileSync(path.join(ROOT, "data", "schedule.json"), JSON.stringify({ total: TOTAL, shots, scenes, cues }, null, 1) + "\n", "utf8");

console.log(`total ${TOTAL}s · ${scenes.length} scenes · ${cues.length} cues`);
