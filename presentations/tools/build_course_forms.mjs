...
// 說明 text are ours.
//
// The 評分表 is the reviewing teacher's document: 分數、等級、SDG 勾選與建議事項全部留空。
// We fill only the header (團隊成員／指導教授／專題名稱) so the teacher knows whose work it is.
// 自評表才是我們填的。
...
/** ticked: 自評表帶 SDG_TICKED；評分表帶空陣列，勾選交給評審老師。 */
function sdgBlock(ticked) {
  return `    <div class="sdg">
      <div class="t">請勾選與專題內容相關之 SDG</div>
      <ul>
${SDGS.map((s, i) => `        <li class="${ticked.includes(i) ? "on" : ""}"><span class="box">${ticked.includes(i) ? "✔" : ""}</span>${esc(s)}</li>`).join("\n")}
      </ul>
    </div>`;
}
...
<!--
  ...
  （docs/course_forms/，IEET 20230317 版）。${self
    ? "自評表我們填：團隊成員／指導教授／專題名稱、自評等級與說明、以及與專題相關的 SDG。"
    : "評分表我們只填表頭（團隊成員／指導教授／專題名稱）；\n  分數、等級、SDG 勾選與建議事項一律留空，那是評審老師要填的。"}
-->
<html lang="zh-Hant">
  <head>
    <meta charset="UTF-8" />
...
${items}
${sdgBlock(self ? SDG_TICKED : [])}${advisory}
  </body>
</html>
`;
}
